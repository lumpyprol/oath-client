...
- [x] every section of the Law (§1–§11) is implemented or explicitly
      ...
      P3 added the other half: verify the claim against the LAW TEXT too,
      after a correctly-implemented ruling turned out to be wrong about
      §3.2 — see D48)
...
**Prompt plan.** `prompt_plan_phase_3.md` — 10 TDD units (09-12). The
design core is units 4–7 (two-visit campaign, two windows, standing
responses, one-visit defence); unit 9 is the measured 6-player acceptance.
...
- **NEEDS DOING — Law §1.23's setup choices, currently made FOR the player
  ...
  (Glossary "Discard")
- **NEEDS DOING — §5.5.3's Citizen-ally window (from P2).** With one
  response phase, a Citizen's permission arrives in the very action that
  closes the window, so only the mandatory Chancellor Ally can ever act
  in it. Plan unit 5 gives the Law its two windows (§5.5.2 join, then
  §5.5.3 battle plans).

**Decisions made at planning (09-12, see D49–D54).**
- Interrupts stay ordinary pending decisions resolved by logged actions;
  P3 ships contracts, not a subsystem — a normative `INTERRUPTS.md`
  enforced by conformance tests (D49)
- The batching floor is information reveals: consecutive choices by one
  player collapse into one action unless a reveal (dice, draw) separates
  them. Wake becomes one action; sacrifice + seizure ride one post-roll
  action; Search stays two (D50)
- Campaign dice roll in the `prepare()` of the action that closes the
  response window (respond; declare vs bandits); `campaign.roll` is
  deleted. Attacker: two visits; defender: one. Fallback recorded if
  this fights replay: keep roll, batch the rest (D51)
- Standing responses are state written by a logged `standing.set` action,
  consulted purely at raise points — the short-circuit never appends an
  action; global-only in P3; revocation affects future raises only;
  timeouts still never auto-resolve (D52)
- Log compatibility: through P3, shape changes are legal at the cost of
  deliberate fixture regeneration and draining the Fly smoke games; from
  P4 the log format freezes and changes need a migration story (D53)
- §1.23's setup choices become from-init, sequential, batched pending
  decisions; back-compat via an open/applied flag on `OathSetup`, so old
  stored setups and committed fixtures fold unchanged (D54)
...
- **Law review (09-11, D43) — the sweep that added plan units 16b–16d.**
  ...
  **scheduled into P4 (09-12)**, not merely parked near it — see P4's
  ...
  them unclaimed is a real change to the economy, not a skipped option; **battle
  ...
  database, so the play paths can't enforce them. **Resolved 09-12
  (Q12/D46): scoped into unit 19** rather than deferred wholesale — the
  data exists in no machine-readable source (the card CDN has no
  restriction field; it is an icon on the card face), so only the cards
  the acceptance game actually plays get transcribed, into an addendum
  where ABSENT means "unread" rather than "unrestricted". Everything
  else stays self-policed until v2.
...
- **Reaction windows (09-12, P3 planning).** A general "anyone want to
  declare a reaction?" window is v2 work: in v1 no card text is enforced,
  so the campaign response window (two-phased by P3 unit 5) is the only
  reactive window that exists. When v2 enforces a card with a reactive
  power, it gets a window here — surfaced as a batched prompt with a pass
...
| D49 | 09-12 | P3 planning: interrupts remain ordinary pending decisions resolved by ordinary logged actions — no decision queue, no second dispatcher. What P3 adds is CONTRACTS: a normative `INTERRUPTS.md` cataloguing every decision kind (raiser, owner, resolvers, locking, id anchor, batching/standing status), enforced bidirectionally by conformance tests (every emitted kind is catalogued; every catalogued built kind is emitted and resolved somewhere in the suite; ids unique per state, pure across refolds, stable across polls), plus a visits-per-turn metric (a visit = a maximal run of consecutive same-actor actions) with the P2 baseline recorded in the doc | The P2 shape already gives rewind, replay, projection and deep-linkability for free; a parallel interrupt subsystem would duplicate all four. The catalogue-with-teeth pattern is RULES-COVERAGE.md's (D48), which caught what prose notes never did | active |
| D50 | 09-12 | The batching floor is information reveals: one player's consecutive choices collapse into a single action's payload unless separated by a reveal they had to see first (dice faces, drawn cards). Applied: the Wake Phase is one `wake.resolve` action (up to 3 visits today, no reveal between steps); sacrifice + seizure choices ride one post-roll `campaign.resolve`; Search stays two actions because the draw is a reveal — that two-step is the floor, not a miss | Gives "batched prompts" a principled definition instead of case-by-case judgment: the minimum visits for a flow is reveals + 1, and any flow above that number is a bug by definition — which is what lets unit 9 treat a visit spike as a finding | active |
| D51 | 09-12 | Campaign dice roll in the `prepare()` of whichever action CLOSES the response window — `campaign.respond` normally, `campaign.declare` itself against bandits — and `campaign.roll` is deleted. The faces land in the closing action's payload; dice are public, so the log stays readable and D14's rationale is untouched. Attacker visits per campaign drop from three (roll, resolve, seize) to two (declare; resolve-with-seizure); with a standing defence (D52) the closer can be declare or the last ally answer, keeping D14's rule that dice never roll in a reducer. Recorded fallback if this fights the store or replay: keep `campaign.roll` and take only D50's batching — two attacker visits on defence instead of one, still far under baseline | The attacker's pure "come online to roll" visit is wall-clock waste in exactly the flow the HLD's interrupt constraint names; the roller's identity carries no game meaning, only the faces do — and those are decided once and persisted wherever they roll | active |
