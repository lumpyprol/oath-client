...
// (src/index.ts). MIT — see vendor/oathparser/LICENSE and
// vendor/oathparser/PROVENANCE.md.
//
// Changes from upstream, all mechanical for `strict` compilation — parsing and
// serialization behaviour is unchanged:
//   - split the `./interfaces` barrel import into explicit .js imports
//   - `parseColorByte` return type widened to `PlayerColor | undefined`
//   - `colorMask`'s lookup table given an explicit type
//   - `Oath[game.oath]` indexed via `keyof typeof Oath`
//   - removed a stray `console.log` in parseData_3_1_1
import { Citizenship, Oath, PlayerColor } from './enums.js';
import type { PlayerCitizenship, Suit } from './enums.js';
import { CardName, CardNameIndexes } from './names.cards.js';
import { SiteName, SiteNameIndexes } from './names.sites.js';
import type { Card, OathGame } from './oathgame.js';
...
function parseColorByte(byte: number): PlayerColor | undefined {
...
function colorMask(color: PlayerColor): number {
  const masks: Record<PlayerColor, number> = {
...
export function serializeOathGame(game: OathGame): string {
  ...
  const basicData: string[] = [
    ...
    hex(Oath[game.oath as keyof typeof Oath], 2),
